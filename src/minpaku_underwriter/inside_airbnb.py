...
def haversine_km(lat1: float, lon1: float, lat2: pd.Series, lon2: pd.Series) -> pd.Series:
    ...
    return pd.Series(distance, index=lat2.index)
...
class InsideAirbnbTokyo:
    ...
    def _url(self, snapshot_date: str, kind: str) -> str:
        ...
        return (
            f"https://data.insideairbnb.com/{TOKYO_PATH}/"
            f"{snapshot_date}/data/{kind}.csv.gz"
        )
    ...
    def select_comps(
        ...
        if "last_review" in df:
            ...
            anchor = pd.Timestamp(as_of) if as_of is not None else pd.Timestamp.today().normalize()
    ...
    def review_occupancy_history(
        ...
        if as_of is not None:
            ...
            raise ValueError("No review dates available to anchor history")
        last_month = anchor.to_period("M")
        first_month = last_month - (months - 1)
        rv = rv[
            (rv["date"].dt.to_period("M") >= first_month)
            & (rv["date"].dt.to_period("M") <= last_month)
        ]
        counts = (
            rv.assign(month=rv["date"].dt.to_period("M"))
        ...
        minimum = pd.to_numeric(meta.get("minimum_nights", default_stay), errors="coerce")
        meta["minimum_nights"] = minimum.fillna(default_stay).clip(upper=30)
        prices = meta.get("price", pd.Series(index=meta.index, dtype=object))
        meta["price_yen"] = prices.map(_money)
        ...
        listing_ids = meta["listing_id"].dropna().astype("int64")
        grid = pd.MultiIndex.from_product(
            [listing_ids, months_idx], names=["listing_id", "month"]
        ).to_frame(index=False)
        grid = grid.merge(meta, on="listing_id", how="left")
        grid = grid.merge(counts, on=["listing_id", "month"], how="left")
        ...
        grid["days"] = grid["month"].map(
            lambda p: month_calendar.monthrange(p.year, p.month)[1]
        )
        ...
        rows = []
        for period, group in grid.groupby("month"):
            x = group["occupancy_proxy"].to_numpy(dtype=float)
            weights = group.get(
                "comp_weight", pd.Series(np.ones(len(group)), index=group.index)
            ).fillna(1)
            ...
            rows.append(
                {
                    ...
                    "occupancy_mean": mean,
                    "occupancy_std": (
                        float(np.nanstd(x, ddof=1)) if len(x) > 1 else 0.0
                    ),
                    ...
                    "adr_proxy_yen": (
                        float(np.nanmedian(group["price_yen"]))
                        if group["price_yen"].notna().any()
                        else float("nan")
                    ),
                    "revenue_proxy_yen": float(
                        np.nansum(group["revenue_proxy_yen"] * w)
                    ),
    ...
    def calendar_unavailability(
        ...
        result = (
            cal.groupby("month")
            .agg(
                unavailable_rate=("unavailable", "mean"),
                comp_calendar_rows=("listing_id", "size"),
            )
            .reset_index()
        )
